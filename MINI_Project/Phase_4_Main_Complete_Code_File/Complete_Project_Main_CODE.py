...
def main():
    ...
    scale = 80  # Initial scale of the cube
    ...
    while cap.isOpened():
        ...
        if results.right_hand_landmarks or results.left_hand_landmarks:
            ...
            if results.left_hand_landmarks:
                ...
                thumb_tip = (left_hand_landmarks[4].x * frame.shape[1], left_hand_landmarks[4].y * frame.shape[0])
                index_tip = (left_hand_landmarks[8].x * frame.shape[1], left_hand_landmarks[8].y * frame.shape[0])
                
                # Calculate the distance between thumb and index finger
                thumb_index_distance = ((thumb_tip[0] - index_tip[0])**2 + (thumb_tip[1] - index_tip[1])**2)**0.5
                
                # Adjust the scale of the cube based on the distance
                scale_factor = 1
                scale = max(1, min(200, int(thumb_index_distance * scale_factor / 1.05)))  # Scale range from 1 to 200
        ...
        screen.fill(black)
        draw_3d_cube(screen, angle_x, angle_y, angle_z, (screen_width // 4, screen_height // 2),scale)  # Left side
        draw_3d_cube(screen, angle_x, angle_y, angle_z, (screen_width * 3 // 4, screen_height // 2),scale)  # Right side
        draw_3d_cube(screen, angle_x, angle_y, angle_z, (screen_width // 2, screen_height // 4),scale)  # Top side
        draw_3d_cube(screen, angle_x, angle_y, angle_z, (screen_width // 2, screen_height * 3 // 4),scale)  # Bottom side
